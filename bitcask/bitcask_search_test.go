...
func Test_Search(t *testing.T) {
	...
	t.Run("BasicSearch", func(t *testing.T) {
		...
		resChan, errChan := db.With(storename).(encryptdb.Store).Search(needle)
	...
	t.Run("NoResultsSearch", func(t *testing.T) {
		...
		resChan, errChan := db.With(storename).(encryptdb.Store).Search(bogus)
		select {
		case err := <-errChan:
...
func Test_ValueExists(t *testing.T) {
	...
	t.Run("ValueExists", func(t *testing.T) {
		...
		for _, ndl := range needles {
			k, exists := db.With(storename).(encryptdb.Store).ValueExists(ndl)
			if !exists {
	...
	t.Run("ValueShouldNotExist", func(t *testing.T) {
		for n := 0; n != 5; n++ {
			...
			if _, exists := db.With(storename).(encryptdb.Store).ValueExists([]byte(garbage)); exists {
...
func Test_PrefixScan(t *testing.T) {
	...
	var needles = []keyvalue.KeyValue{
		keyvalue.NewKeyValue(keyvalue.NewKey([]byte("user:Frickhole")), keyvalue.NewValue([]byte(c.RandStr(55)))),
		keyvalue.NewKeyValue(keyvalue.NewKey([]byte("user:Johnson")), keyvalue.NewValue([]byte(c.RandStr(55)))),
		keyvalue.NewKeyValue(keyvalue.NewKey([]byte("user:Jackson")), keyvalue.NewValue([]byte(c.RandStr(55)))),
		keyvalue.NewKeyValue(keyvalue.NewKey([]byte("user:Frackhole")), keyvalue.NewValue([]byte(c.RandStr(55)))),
		keyvalue.NewKeyValue(keyvalue.NewKey([]byte("user:Baboshka")), keyvalue.NewValue([]byte(c.RandStr(55)))),
	}
	...
	resChan, errChan := db.With(storename).(encryptdb.Store).PrefixScan("user:")
	var results []keyvalue.KeyValue
	for keyValue := range resChan {
