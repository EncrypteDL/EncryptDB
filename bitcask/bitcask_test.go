...
func newTestDB(t *testing.T) (string, encryptdb.Keeper) {
	t.Helper()
	tpath := t.TempDir()
	tdb := OpenDB(tpath)
	if tdb == nil {
...
func seedRandKV(db encryptdb.Keeper, store string) error {
	return db.With(store).Put([]byte(c.RandStr(55)), []byte(c.RandStr(55)))
}

func seedRandStores(db encryptdb.Keeper, t *testing.T) []string {
	t.Helper()
	names := make([]string, 0)
	for n := 0; n != 5; n++ {
