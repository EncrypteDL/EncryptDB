...
func NewMigrator(from, to encryptdb.Keeper) (*Migrator, error) {
...
func (m *Migrator) CheckDupes() error {
	...
	for storeName, store := range fromStores {
		...
		wg.Add(1)
		go func(storeName string, store, existingStore encryptdb.Filer) {
			defer wg.Done()
...
func (m *Migrator) Migrate() error {
	...
	for srcStoreName, srcStore := range fromStores {
		...
		wg.Add(1)
		go func(storeName string, store encryptdb.Filer) {
			defer wg.Done()
			keys := store.Keys()
			for _, key := range keys {
				select {
