		...
		t.Run(name+"_basic", func(t *testing.T) {
			...
			t.Run("sanity_check", func(t *testing.T) {
				...
				if nonexistentValue, shouldError := instance.With(storeName).Get(testKey); shouldError == nil || nonexistentValue != nil {
					...
					if !keyvalue.IsNonExistenKey(shouldError) {
...
func insertGarbo(t *testing.T, db encryptdb.Keeper) map[string][]keyvalue.KeyValue {
	t.Helper()
	inserted := make(map[string][]keyvalue.KeyValue)
	for i := 0; i < 10; i++ {
		...
		for j := 0; j < 100; j++ {
			...
			inserted[newName] = append(inserted[newName], keyvalue.NewKeyValueFromBytes(key, value))
		}
