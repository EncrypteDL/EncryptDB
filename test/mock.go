...
var (
	mockKeepers = make(map[string]map[string]encryptdb.Filer)
	mockMu      sync.RWMutex
)
...
func (m *MockKeeper) WriteMeta(path string) error {
	...
	mockMu.Unlock()
	registry.RegisterKeeper(m.name, func(path string, opts ...any) (encryptdb.Keeper, error) {
...
type MockKeeper struct {
	...
	path    string
	defOpts []MockOpt
	stores  map[string]encryptdb.Filer
	mu      sync.RWMutex
}
...
func NewMockKeeper(name string, defopts ...any) *MockKeeper {
	...
	mk := &MockKeeper{
		name:   name,
		stores: make(map[string]encryptdb.Filer),
	}
...
func (m *MockKeeper) With(name string) encryptdb.Filer {
	m.mu.RLock()
	s, ok := m.stores[name]
	m.mu.RUnlock()
	if !ok {
...
func (m *MockKeeper) AllStores() map[string]encryptdb.Filer {
	m.mu.RLock()
	stores := make(map[string]encryptdb.Filer, len(m.stores))
